...
namespace Hyperion.Tests
{
    public class CollectionTests : TestBase
    {
        ...
        [Fact]
        public void CanSerializeCombGuidDictionary()
        {
            var expected = new Dictionary<CombGuid, string>
            {
                [CombGuid.NewComb()] = "def",
                [CombGuid.NewComb()] = "jkl,"
            };

            Serialize(expected);
            Reset();
            var actual = Deserialize<Dictionary<CombGuid, string>>();
            Assert.Equal(expected.ToList(), actual.ToList());
        }
