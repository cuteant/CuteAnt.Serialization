...
namespace ServiceStack
{
    ...
    {
        internal static readonly ConcurrentDictionary<Tuple<Type, Type>, MemberGetter> converters
            = new ConcurrentDictionary<Tuple<Type, Type>, MemberGetter>();
        ...
        public static MemberGetter GetConverter(Type fromType, Type toType)
        {
        ...
        public static object ConvertTo(this object from, Type toType, bool skipConverters)
        {
            ...
            if (from is ReadOnlyMemory<char> rom)
                return SSTTypeSerializer.DeserializeFromSpan(toType, rom.Span);
        ...
        public static object ChangeValueType(object from, Type toType)
        {
            ...
            if (s != null)
                return SSTTypeSerializer.DeserializeFromString(s, toType);
            ...
            return SSTTypeSerializer.DeserializeFromString(from.ToJsv(), toType);
        }
    ...
    internal class AssignmentDefinition
    {
        ...
        public void PopulateFromPropertiesWithAttribute(object to, object from, Type attributeType)
        {
            var hasAttributePredicate = (Func<PropertyInfo, bool>)
                (x => x.GetAllAttributes(attributeType).Any());
    ...
    internal static class TypeConverter
    {
        public static MemberGetter CreateTypeConverter(Type fromType, Type toType)
        {
            ...
            if (toType == typeof(string))
                return o => SSTTypeSerializer.SerializeToString(o).StripQuotes();
            ...
            else 
            {
                return fromValue =>
                {
                    ...
                    var toValue = ActivatorUtils.FastCreateInstance(toType);
